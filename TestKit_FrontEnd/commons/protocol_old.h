...
uint32_t source ;		// source signature
...
unsigned short length ; 	// total length of this frame (CRC included)
uint32_t source ;		// source signature
uint32_t destination ;	// destination signature
...
uint32_t source_num ;		// source signature number
uint32_t source_id ;		// source signature id
uint32_t dest_id ; 			// destination signature id
...
unsigned char length ;		// total length of this identifier
uint32_t source_ip ;		// destination ip address
} __attribute__ ((packed)) ;
...
struct CMD_BOOT {
uint32_t act_time ;	// actual time in sec. from Jan. 1, 1970 at 0 a.m.
uint32_t off_time ;	// turn off time in sec. from Jan. 1, 1970 at 0 a.m.
uint32_t sernum ;		// device serial number
...
struct GPS_FULLFIX {
uint32_t ftime ;		// time of fix in sec. from Jan. 1, 1970 at 0 a.m.
short speednorth ;			// speednorthKmh = speednorth * 0.072
short speedeast ;			// speedeastKmh  = speedeast  * 0.072
int32_t ilat ;					// lat=(double)(ilat)/(double)(0x40000000L)*90.0
int32_t ilon ;					// lon=(double)(ilon)/(double)(0x40000000L)*90.0
...
uint32_t cnt1 ;		// raw counter 1
uint32_t cnt2 ;		// raw counter 2
} __attribute__ ((packed)) ;
...
uint32_t km ;			// space counter, in meters
uint32_t time_In0 ;	// minutes with In0 active
}  __attribute__ ((packed)) ;

struct GPS_TRACK {
uint32_t stime ;		// start time (0=now)
uint32_t etime ;		// end time (0=never, -1=stop)
...
uint32_t num_fix ; 	// num of GPS fix
unsigned short num_term ;	// num of Terminal insertions
uint32_t tel_oper ;	// telephone operator
...
uint32_t thr_low  ; // soglia minima (valore come da can)
uint32_t thr_high ; // soglia minima (valore come da can)
} __attribute__ ((packed)) ;	// size is 8

struct W_CAN_MULTISMBIT{
uint32_t bitmask ; // mask   su SmCanBits
uint32_t bitvals ;// valore di SmCanBits
} __attribute__ ((packed)) ;	// size is 8
...
uint16_t   send_time ;     // 0 = no send
uint32_t		address ;
uint32_t		addr_mask ;     // 0x8XXXXXXX -> Standard
uint32_t		tag ;
unsigned char   flags ;		
