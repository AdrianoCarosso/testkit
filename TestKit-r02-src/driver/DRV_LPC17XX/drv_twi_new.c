...
#define ACC_SAMPLE_FREQ     400                     // in Hz
//#define ACC_SAMPLE_FREQ     100                     // in Hz
...
//		tw1fasterr++ ;
        if (twi1buflen) {
            ...
            *twi1bufptr++ = I2Cx->I2DAT ;
            //*twi1bufptr = I2Cx->I2DAT ;
			//if ( (twi1buflen==4) && (*twi1bufptr) ) tw1fasterr++ ;
// 			if (!tw1old){
// 				if (*twi1bufptr==tw1old) tw1fasterr++ ;
// 			}
// 			tw1old = *twi1bufptr ;
			//if (twi1buflen<7) twi1bufptr++ ;
            twi1buflen-- ;
            if (twi1buflen != 0) {
                PUT_DEBUG('b');
                I2Cx->I2CONSET = I2CONSET_AA ;  // assert ACK after data is received
...
void twistart(void)
{
    ...
    // configure: set clock
    I2C0->I2SCLL = current_clock/4/50000 / 2 ;
    I2C0->I2SCLH = current_clock/4/50000 / 2 ;
...
// #else
	SC->PCLKSEL1 &= ~0x000000c0 ;       // bit7,6: 00 - PCLK = CCLK / 4
// // FR_PROVA
//  	SC->PCLKSEL1 |= 0x00000080 ;       // bit7,6: 10 - PCLK = CCLK / 2
//#endif
    ...
    // configure: set clock
    I2C1->I2SCLL = current_clock/4/50000 / 2 ;
    I2C1->I2SCLH = current_clock/4/50000 / 2 ;
// // FR_PROVA
// //    I2C1->I2SCLL = current_clock/4/100000 / 2 ;
// //    I2C1->I2SCLH = current_clock/4/100000 / 2 ;
//     I2C1->I2SCLL = current_clock/4/50000 / 4 ;
//     I2C1->I2SCLH = current_clock/4/50000 / 4 ;
...
{
#ifdef USE_TW1_FAST_ACCELEROMETER
    if (tw1fastbuffer) return(2) ; // ERROR) ;  // fast mode enabled
#endif // USE_TW1_FAST_ACCELEROMETER
    twi1askrestart = 0 ;         // clear restart flag
...
#ifdef CBUG_
        if (tw1fasterr){
			pdebugt(1,"ERRFAST %d", tw1fasterr) ;
			tw1fasterr = 0 ;
		}
#endif // CBUG
