...
// 21/10/2011 Aggiunta RF
unsigned char COMS_disable ;
//
...
// _FR_
    ...
    case 0:
        THISUART = UART0 ;
#ifdef USECOM2_RTSCTS
		if (SC->PCONP & PCONP_PCUART0) while(KS_dequeue(COM0OQ, &c) !=RC_QUEUE_EMPTY) ;
    ...
    case 2:
        THISUART = UART2 ;
#ifdef USECOM2_RTSCTS
		if (SC->PCONP & PCONP_PCUART2) while(KS_dequeue(COM2OQ, &c) !=RC_QUEUE_EMPTY) ;
        ...
        case COM2ISEM :                 // data arrived         // <-- COMx
            {
                ...
                while(rxiptr[id] != rxcptr[id]) {
#ifdef USECOM0_RTSCTS_
					if((qheader[COM2IQ].curndx+30)> qheader[COM2IQ].depth) COM0_SETRTS ;
#endif
