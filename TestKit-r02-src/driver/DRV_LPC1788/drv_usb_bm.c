...
#define TMINT_USBTASK ((TICKS)10*CLKRATE/1000)      // Wake up every 20 msec
static char localusb[64] ;
...
void usbtask(void)
{
    ...
    for( ; ; ) {
        ...
        case USBQSEM :  // something to send
            while(KS_dequeue(USBOQ, &localusb[0]) == RC_GOOD) { 
                i = 1 ;
                while(KS_dequeue(USBOQ, &localusb[i]) == RC_GOOD) { 
                    i++ ;
                    if (i >= sizeof(localusb))
                        break ;
                }
                if (CDC_Device_SendData(&VirtualSerial_CDC_Interface, localusb, i)) {
                    usberr++ ;
#ifdef CBUG   
                    pdebugt(1, "CDC_Device_SendData error") ;
#endif // CBUG    
                }
            }            
            // if (CDC_Device_Flush(&VirtualSerial_CDC_Interface)) usberr++ ;
            break ;
                
        case USBTIMER :
            // check if some byte received and pending in buffer
            for(i=0 ; i<3 ; i++) {
                // not all the times the answer is correct
                while( ((rtx_byte = CDC_Device_ReceiveByte(&VirtualSerial_CDC_Interface)) != -1) ) {
#ifdef CBUG    
                    // pdebugt(1, "received 0x%02x - %c", rtx_byte, (rtx_byte>=' ') ? rtx_byte : '_') ;
#endif // CBUG    
