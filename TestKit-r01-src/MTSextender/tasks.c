...
void ParseBuffer(const char *usbbuf){
  ...
  switch(usbbuf[0]) {
    ...
    case 'D' :  // dump
        // digital I/O
        printf("D=%08lx,%08lx,%08lx,%08lx;", dio_read(PORT_PIOA), dio_read(PORT_PIOB), dio_read(PORT_TW1), dio_read(PORT_TW2)) ;
    ...
    case 'd' :  // dump
        // digital I/O
        printf("PIOA=0x%08lx, PIOB=0x%08lx, TW1=0x%08lx, TW2=0x%08lx\n", dio_read(PORT_PIOA), dio_read(PORT_PIOB), dio_read(PORT_TW1), dio_read(PORT_TW2)) ;
        // read analog inputs from ADC -0- and -1-
        for(i=0 ; i<19 ; i++) {
            if (i) printf(", ") ;
...
//        AT91C_BASE_PIOB->PIO_SODR = outmask & outval ;          // set at 1
    ...
    case 'a' :  // set CAN baud
        ...
        if ((i>=0) && (i<=CAN_TOT_CHANNELS)) {
            dio_write(PORT_PIOB, PIOB_CAN_OFF, PIOB_CAN_OFF) ;
		...
		while(usbbuf[i]){	// Scan all command
			switch (usbbuf[i]){
				...
				case 'I':	// Digital channel out (In for MTS)
					if (usbbuf[i]=='A'){
						dio_write(PORT_PIOB, 0x8, 0x8) ; // First set Analog (PB3 to 1)
						chn = 0 ;
					}else{
						dio_write(PORT_PIOB, 0x8, 0x0) ; // First set Digital (PB3 to 0)
						chn = 1 ;
					}
					i++ ;
					if (usbbuf[i]=='V'){  // AD as voltage, I/O at Vext (A30)
						dio_write(PORT_PIOA, 0x40000000, 0x40000000) ; 
					}else{
						dio_write(PORT_PIOA, 0x40000000, 0x0) ;
					}
					i++ ;
					r = atoi(&usbbuf[i]) ;
					if (chn) r = 9-r ;
					i++ ;
					if (r & 0x1){ // If odd
						dio_write(PORT_PIOA, 0x800000, 0x800000) ;
					}else{
						dio_write(PORT_PIOA, 0x800000, 0x0) ;
					}
					switch(r){
						case 1:
						case 2:
							dio_write(PORT_PIOB, 0x202, 0x202) ;
							break ;
						case 3:
						case 4:
							dio_write(PORT_PIOB, 0x202, 0x2) ;
							break ;
						case 5:
						case 6:
							dio_write(PORT_PIOB, 0x202, 0x200) ;
							break ;
						case 7:
						case 8:
						case 0:
							dio_write(PORT_PIOB, 0x202, 0x0) ;
							break ;
				...
				case 'P':	// presence
					i++ ;
					if (usbbuf[i]=='0')
						dio_write(PORT_PIOB, 0x4, 0x4) ; // pioB 2 to 1
					else
						dio_write(PORT_PIOB, 0x4, 0x0) ; // pioB 2 to 0
					break ;
				case 'V':	// Vext
					i++ ;
					if (usbbuf[i]=='0')
						dio_write(PORT_PIOB, 0x20, 0x20) ; // pioB 5 to 1
					else
						dio_write(PORT_PIOB, 0x20, 0x0) ; // pioB 5 to 0
					break ;
			}
			i++ ;
		}
