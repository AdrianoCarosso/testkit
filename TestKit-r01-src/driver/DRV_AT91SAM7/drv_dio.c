...
// #endif
	...
	lbuf[0] = 0xff ; // all output
	TWI_txrx(PCAL9555A_ADDR2, 0x2, lbuf, 2) ;
  ...
  #else
	lbuf[0] = 0xff ; // all output
	TWI_txrx(PCAL9555A_ADDR2, 0x3, lbuf, 2) ;
  #endif
#endif
...
//      0       internal PORT A, 32 bit
//      1       internal PORT B, 32 bit
//      2       external optional MAX7324 TW1, 8 output data bit OR PCAL9555A
...
void dio_write(int port, int pmask, int pval){
  ...
  switch(port) {
    ...
    case PORT_TW1 :
      lbuf[0] = 0x02 ;
      lbuf[1] = (pmask & pval) & 0xff;
      TWI_send(PCAL9555A_ADDR, lbuf, 2) ;
      lbuf[0] = 0x03 ;
      lbuf[2] = ((pmask & pval)>>8) & 0xff ;
      TWI_send(PCAL9555A_ADDR, lbuf, 2) ;
      break ;
